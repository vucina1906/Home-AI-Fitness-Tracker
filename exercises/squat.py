...
# Function to calculate responsiveness based on knee angle
def calculate_responsiveness(angle, min_angle=90, max_angle=140):
    if angle < min_angle:
        return 0  # Minimum responsiveness for lower knee angles
    elif angle > max_angle:
        return 1  # Maximum responsiveness for higher knee angles
    else:
    ...
    with mp_pose.Pose(min_detection_confidence=0.7, min_tracking_confidence=0.7) as pose:
        ...
        while True:
            ...
            if results.pose_landmarks:
                ...
                if (left_knee_angle < 90 and right_knee_angle < 90 and 
                ...
                # Stand Up Condition
                if (left_knee_angle > 140 and right_knee_angle > 140 and 
                    left_elbow_angle < 90 and right_elbow_angle < 90):
                    if direction == 1:
                        counter += 0.5
                        direction = 0
                ...
                # Draw all landmarks, differentiating angle-related landmarks with a green glow
                for idx, landmark in enumerate(landmarks):
                    center = (int(landmark.x * w), int(landmark.y * h))
                    if idx in [25, 26, 27, 28, 23, 24]:  # Landmarks used for knee angles
            ...
            cv2.imshow("Squat Exercise", frame)
